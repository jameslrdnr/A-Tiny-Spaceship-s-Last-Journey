...
//        BASICPOINTPICKUPSCREENOBJECTID = Byte.parseByte(OBJECTIDMAP.getProperty("BASICPOINTPICKUPSCREENOBJECTID"));
    ...
    */
    ...
    //gets distance to a targeted object
    public float getDistanceToTarget(AbstractScreenObject target){
        float yDist = target.getY() - getY(), xDist = target.getX() - getX();
        
        return (float)Math.sqrt((yDist * yDist) + (xDist * xDist));
    }
    
    //get degrees to rotate towards a certain target
