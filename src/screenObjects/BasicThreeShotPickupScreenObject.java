...
public class BasicThreeShotPickupScreenObject extends BasicPickupScreenObject {
    ...
    public void init(){
        c = true;color1 = new Color(32, 178, 170);
        ...
        getParticleSys().setParticleFade(true);getParticleSys().setColor(color1);getParticleSys().setIsVisible(true);getParticleSys().setParticleSpeedVariance(0);
    }

    @Override public void runLogic(){
    ...
    @Override public void drawObject(Graphics2D g){
        ...
        /*g.fillRect((int)getX(), (int)getY() + getHeight()/2 - getHeight()/12, getWidth(), getHeight()/6);/*g.fillRect((int)getX() + getHeight()/2 - getHeight()/12, (int)getY(), getWidth()/6, getHeight());*/
        if(Debug.isEnabled()){
            g.setColor(Color.RED);
            g.draw(getCollisionShape());
        }
    }
    /**** @return*/@Override public boolean shouldDestroyObject(){
